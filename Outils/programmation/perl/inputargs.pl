...
## Test existance carac et carac.inc, overwrite éventuel
  if ($file{'carac.inc'}) {
    ...
    #system ( "cp", "./carac_template.inc", $path."carac.inc") if ($choice eq 'yes'); # Ici, je pense qu'il fait le system cp dans tous les cas (yes no)
    print "carac.inc file unchanged\n" if ($choice eq 'no');
  } else {
...
#Traitement des step param
      ...
      print "\tUser pvt command and step parameter on voltage/temperature. Priority on pvt => step removed form carac file\n" and next if ( ($1 =~ /vddgo1|vddgo2|vddio|tval/) && @pvt ) ; 
      ...
      push @step , split / /,$1;
      my $grep = grep { exists $step_param{$_} } @step; ## test d'existance du step en commande. Ici, cela ne marche pas super quand on passe par .step param vddgo1 et qu'on a fait un -voltage qqchose, parce qu'il n'y a pas de step_param dessu ....
      print "\tUser defined step on this parameter ; This step will be removed from carac file\n" and next if ( $grep>0 );
      print "\tUser pvt command and step parameter on voltage/temperature. Priority on pvt => step removed form carac file\n" and next if ( ($1 =~ /vddgo1|vddgo2|vddio|tval/) && @pvt ) ; 
      print ( $eldoFH $_ ) and next;
...
#Ecriture de la liste des corners dans le cas de corners multiples : .alter nécessaire
  if (scalar(@corner_mos) >= 1 || scalar (@corner_bip) >= 1 || scalar (@corner_res) >= 1 || scalar(@corner_cap) >= 1) {
    push @corner_mos,"mos_typ" if scalar(@corner_mos) == 0;
...
sub f_process_help() {
  ...
  print ("     usage : process=mos_typ/mos_fs/mos_sf/mos_ss/mos_ff/btyp/bmin/bmax/rtyp/rmin/rmax/ctyp/cmin/cmax\n");
...
if (@process) {
  ...
  @corner_mos = grep ( /mos_typ|mos_ff|mos_ss|mos_fs|mos_sf/i , @process);
...
if (@pvt) {
  ...
  @corner_mos = qw(mos_typ mos_ff mos_ss mos_fs mos_sf);
